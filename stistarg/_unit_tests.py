...
class TestCheckbox(object):
    ...
    def test1(self):
        ...
        stistarg_results = stistarg(filename, source='point', checkboxsize=3, display=True)
    ...
    def test2(self):
        ...
        fits.writeto(filename, arr, overwrite=True)
        stistarg_results = stistarg(filename, source='point', checkboxsize=3)
    ...
    def test5(self):
        ...
        arr = gaussarr(15, 30, fwhm=10, height=100) 
        fits.writeto(filename, arr, overwrite=True)
        stistarg_results = stistarg(filename, source='diffuse', checkboxsize=25)
        assert_almost_equals( stistarg_results['fluxCentroid'][0], 16 - 1., places=5)
        assert_almost_equals( stistarg_results['fluxCentroid'][1], 31 - 1., places=5)
    ...
    def test14(self):
        ...
        fits.writeto(filename, arr, overwrite=True)
        stistarg_results = stistarg(filename, source='diffuse', checkboxsize=33)
