...
stistarg.__globals__['WAIT'] = lambda x: x
...
class TestCheckbox(object):
    ...
    def setup_class(cls):
        ...
        # Put these import here so they get called before importing stistarg
        import matplotlib
    ...
    def test1(self):
        ...
        stistarg_results = stistarg(filename, source='point', checkboxsize=3, display=True, ext=0)
    ...
    def test2(self):
        ...
        fits.writeto(filename, arr, overwrite=True)
        stistarg_results = stistarg(filename, source='point', checkboxsize=3, ext=0)
    ...
    def test5(self):
        ...
        arr = gaussarr(15, 30, fwhm=10, height=100) 
        fits.writeto(filename, arr, overwrite=True)
        stistarg_results = stistarg(filename, source='diffuse', checkboxsize=25, ext=0)
        assert_almost_equals( stistarg_results['fluxCentroid'][0], 16 - 1., places=5)
        assert_almost_equals( stistarg_results['fluxCentroid'][1], 31 - 1., places=5)
    ...
    def test14(self):
        ...
        fits.writeto(filename, arr, overwrite=True)
        stistarg_results = stistarg(filename, source='diffuse', checkboxsize=33, ext=0)
